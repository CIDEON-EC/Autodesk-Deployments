...
## [2.0.1] - 2026-08-18
### Security
- **`UnknownError` is no longer an accepted Authenticode status** in `Install-ADSK.ps1` and `Copy-Local.ps1`: it is a catch-all status that carries no guarantee the file hash was verified, so it cannot substitute for a hash check. Only `Valid` and `NotTrusted` are accepted, both still gated by the pinned signer thumbprint
...
- The module cache folder under `ProgramData` is given a restricted ACL (SYSTEM and Administrators only, inheritance disabled) on every run rather than only when the folder is first created, so a folder left over from an earlier run or pre-created by a non-admin is hardened too to prevent non-admin users from swapping the cached module between signature validation and import
...
- **BREAKING**: The code-signing certificate is no longer installed into `LocalMachine\Root`. Module authenticity is verified via the pinned signer thumbprint allowlist instead of Authenticode chain trust; only the status `NotTrusted` is accepted in addition to `Valid`, and only when the signer thumbprint is pinned (`NotSigned`, `HashMismatch`, `UnknownError` and all other statuses are rejected). Existing machines keep any previously installed Root certificate; new installs no longer add one.
